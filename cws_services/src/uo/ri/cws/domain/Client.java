...
public class Client extends BaseEntity {
	...
	public boolean eligibleForRecommendationVoucher() {
		return hasPaidWorkOrder() && sponsored.stream()
				.filter(recommendation -> recommendation.isValidForVoucher()).collect(Collectors.toList()).size()>=3;
	}
	...
	public List<Recommendation> getRecommendationsValidForVoucher() {
		...
		.filter(recommendation -> recommendation.isValidForVoucher()).collect(Collectors.toList());
	}
